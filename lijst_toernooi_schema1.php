...
<td valign="top" >
<form method = 'post' action='print_toernooi_score_form2.php?toernooi=<?php echo $toernooi_voluit;?>&invul_namen=<?php echo $invul_namen;?>'>
...
<input type ='submit' value= 'Klik hier voor score formulieren'> </center>
</form></td>
</tr>
</table>
...
$baan[$j]           = substr($baan[$j-1],3,$len-3).substr($baan[$j-1],0,3);
...
// zoek positie van team in string kolom1(ronde); tegenstander is overeenkomstige pos in kolom2{ronde), baan is overeenkomstige pos in baan{ronde)
   ...
   for ($k=0;$k<=($aant_wedstr*3);$k=$k+3){        // per stukjes van drie zoeken
  	    ...
  	    $_baan       = sprintf("%d", $_baan); // verwijder voorloopp nullen
 //     echo $team ."<br>";
