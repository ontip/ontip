...
//setcookie ("toernooi", $_GET["toernooi"] , time() +1000);
...
//header('Location: Inschrijfform_mob.php');//
// Database gegevens. 
include('mysql.php');
/// selectie privacy tekst 
$qry1      = mysql_query("SELECT Privacy_tekst From vereniging where Vereniging = '".$vereniging ."' ")     or die(' Fout in select');  
...
if ($bond == 'NJBB'){  
$ver_namen = array();
$qry_v     = mysql_query("SELECT Vereniging_naam from njbb_verenigingen order by Vereniging_naam")     or die(' Fout in select ver');  
//$qry_v     = mysql_query("SELECT Distinct Vereniging, Vereniging_nr From speler_licenties  group by Vereniging")     or die(' Fout in select');  
    while($row = mysql_fetch_array( $qry_v )) {
...
$ver_namen = array();
$qry_v     = mysql_query("SELECT Vereniging From bond_verenigingen  where Bond = '".$bond."'  order by Vereniging ")     or die(' Fout in select bond vereniging');  
    while($row = mysql_fetch_array( $qry_v )) {
...
$update             = mysql_query("UPDATE hulp_toernooi set Inschrijving_open = 'N'  where Vereniging = 'Boulamis' and Toernooi = 'erik_test_toernooi' ");
$qry_toernooien     = mysql_query("SELECT Toernooi,Toernooi_voluit From hulp_toernooi where Vereniging = '".$vereniging ."' and  Inschrijving_open <> 'N' order by Datum ")     or die(' Fout in select 2');  
$aantal_toernooien  = mysql_num_rows($qry_toernooien);
...
if ($toernooi !='') {
	...
	$qry  = mysql_query("SELECT * From config where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."' ")     or die(' Fout in select tornooi');  
...
if (!isset($voucher_code_invoeren_jn)){
 ...
 $qry1              = mysql_query("SELECT * From config where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Variabele = 'voucher_code_invoeren_jn'")     or die(' Fout in select');  
 $result1           = mysql_fetch_array( $qry1);
...
$qry                 = mysql_query("SELECT * From config  where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."' and Variabele = 'tekst_kleur'  ") ;  
$result              = mysql_fetch_array( $qry);
$tekstkleur          = $result['Waarde'];

$qry                 = mysql_query("SELECT * From config  where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."' and Variabele = 'link_kleur'  ") ;  
$result              = mysql_fetch_array( $qry);
$link                = $result['Waarde'];

$qry                 = mysql_query("SELECT * From config  where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."' and Variabele = 'koptekst_kleur'  ") ;  
$result              = mysql_fetch_array( $qry);
...
$sql      = mysql_query("SELECT * from toernooi_datums_cyclus  where  Vereniging_id = ". $vereniging_id." and Toernooi ='".$toernooi."' and Datum >= '".$today."'  order by Datum" )     ; 
...
<FORM action="send_inschrijf1.php" method=post name = "myForm">
...
if (!isset($_GET['simpel'])){ ?>
      ...
      <?php
         while($row = mysql_fetch_array( $qry_toernooien )) {?>
...
/// 11 aug 2015  aanpassing ivm inschrijving beperkt tot 1 vereniging en inschrijving is vol
  ...
  if ($aant_splrs  >= ( $max_splrs + $aantal_reserves ) and $aantal_reserves > 0 ){
   	$vol_geboekt = 1;
...
if ($now > $eind_tijd_inschrijving and $einde == 0){
   ...
   $qry              = mysql_query("SELECT * From config  where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."' and Variabele = 'uitgestelde_bevestiging_jn' ") ;  
   $result           = mysql_fetch_array( $qry);
...
<?php
 ...
 $qry              = mysql_query("SELECT * From config  where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Variabele = 'url_afbeelding' ") ;  
 $result           = mysql_fetch_array( $qry);
...
$qry  = mysql_query("SELECT * from hulp_select_speler 
...
<?php } else { ?>
 ...
 <?php
   ...
   if ($meerdaags_toernooi_jn =='J'){
  	 	 ...
  	 	 $variabele      = 'eind_datum';
       $qry1           = mysql_query("SELECT * From config where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Variabele = '".$variabele ."'")     or die(' Fout in select eind datum');  
       $result         = mysql_fetch_array( $qry1);
       $eind_datum     = $result['Waarde'];
         	...
         	while($row = mysql_fetch_array( $sql )) { 		
                 ...
                 if ($locatie !=''){
                 	$locatie = ".    [".$locatie."]";                                                       
         		     ...
         		     ?>
         		     <input type='checkbox' name='meerdaags_datum[]' value ='<?php echo $_datum;?>'   checked><em><?php echo strftime("%a %e %B ", mktime(0, 0, 0, $maand , $dag, $jaar) );?> <?php echo $locatie;?></em><br>
         		     <?php
         		    }// while	  	
         	?>	    
         		    </tr>
