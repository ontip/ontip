...
// extra invulveld
 ...
 if(isset($extra_invulveld)){
  ...
  $variabele = 'voucher_code_invoeren_jn';
   $qry1      = mysql_query("SELECT * From config where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Variabele = '".$variabele ."'")     or die(' Fout in select voucher code');  
   $result    = mysql_fetch_array( $qry1); 	
...
if ($voucher_code_jn =='J'){
 ...
 // 26 aug 2018
   ...
   $qry1      = mysql_query("SELECT * From config where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Variabele = '".$variabele ."'")     or die(' Fout in select voucher code');  
   $result    = mysql_fetch_array( $qry1); 	
   $email_notificaties_jn        = $result['Waarde'];
...
if (isset($Voucher_code) and $Voucher_code !=''){
   ...
   $qry2      = mysql_query("SELECT * From voucher_codes where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Voucher_code = '".$Voucher_code ."'")     or die(' Fout in select voucher');  
...
if (  $voucher_code_jn =='J'   and $voucher_code_richting =='Uit'){
   ...
   $qry2      = mysql_query("SELECT * From voucher_codes where Vereniging = '".$vereniging ."' and Toernooi = '".$toernooi ."'  and Voucher_gebruikt = 'N' limit ".$aantal_personen." ")     or die(' Fout in select voucher2');  
	 $count     = mysql_num_rows($qry2);
...
$query = "INSERT INTO inschrijf(Id, Volgnummer, Toernooi, Vereniging,Vereniging_id, Datum, 
 ...
 $query="UPDATE inschrijf       SET Voucher_code  = '".$_Voucher_codes."' 
               where Toernooi     = '".$toernooi."' and
                     ...
                     Kenmerk      = '".$kenmerk."'  ";
    //echo $query."<br>";                 
    mysql_query($query) or die ('Fout in update inschrijf : Voucher code'); 
}    
...
// versleutel email
  ...
  $query="UPDATE inschrijf       SET Email_encrypt  = '".$encrypt_email."' , Email ='[versleuteld]' 
               where Toernooi     = '".$toernooi."' and
                     ...
                     Kenmerk      = '".$kenmerk."'  ";
 //   echo $query."<br>";                 
    mysql_query($query) or die ('Fout in update inschrijf : Encrypt email'); 
 }
...
// versleutel telefoon
  ...
  $query="UPDATE inschrijf       SET Telefoon_encrypt  = '".$encrypt_telnr."' , Telefoon ='[versleuteld]' 
               where Toernooi     = '".$toernooi."' and
                     ...
                     Kenmerk      = '".$kenmerk."'  ";
//    echo $query."<br>";                 
    mysql_query($query) or die ('Fout in update inschrijf : Encrypt telnr'); 
 }
...
foreach ($Voucher_code_array as &$Voucher_code) {
   ...
   if (isset($Voucher_code) and $Voucher_code !=''){
       ...
       mysql_query($query) or die ('Fout in update inschrijf : voucher_gebruikt');   
   }
}// end for each
 ...
 if (isset($_POST['meerdaags_datum'])){
  ...
  $query="UPDATE inschrijf       SET Meerdaags_datums   = '".$deelname."' 
               where Toernooi     = '".$toernooi."' and
                     ...
                     Kenmerk      = '".$kenmerk."'  ";
    //echo $query."<br>";                 
   mysql_query($query) or die ('Fout in update inschrijf : Meerdaagse datums');      	
} 
	...
	$status             = 'ID0';
	 $query="UPDATE inschrijf       SET Status = 'ID0' 
               where Toernooi     = '".$toernooi."' and
                     Vereniging   = '".$vereniging."' and
                     Inschrijving = '".$date."'  ";
  mysql_query($query) or die ('Fout in update inschrijf : set Status  = ID0' ); 
}
...
$bericht .= "<tr><td  width=200>Kenmerk    </td><td colspan = 2>"         .  $kenmerk        ."</td></tr>".  "\r\n";
 ...
 if (isset($_POST['meerdaags_datum'])){
  ...
  foreach ($meerdaags_datum as $_datum){
           ...
           $sql2      = mysql_query("SELECT * from toernooi_datums_cyclus  where  Vereniging_id = ". $vereniging_id." and Toernooi ='".$toernooi."' and Datum = '".$_datum."'  " )     ; 
...
// update status voor email notificaties
// 26 aug 2018

if ($email_notificaties_jn =='J'){
    if (isset($_POST['email_notificatie'])){
    	    $email_notificatie = $_POST['email_notificatie'];
          $query="UPDATE email_notificaties set Ingeschreven ='J', Laatst = now()  where Vereniging_id = ".$vereniging_id ." and Toernooi = '".$toernooi ."'  
                    and Notificatie_kenmerk ='".$email_notificatie."'  " ;
 //   echo $query."<br>";                 
   mysql_query($query) or die ('Fout in update email_notificaties'); 
   
   echo "<li style= 'background-color:".$achtergrond_kleur.";font-family:verdana;font-size:10pt;color:".$tekstkleur.";text-decoration:none;'>";
   echo "U zult geen email notificaties meer ontvangen.<br>";
 }    
}
